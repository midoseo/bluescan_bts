...
export function needsAttention(c) {
  // 주의 필요 = 심각 신호 OR 미해결·누적 VOC(주의 고객)만. 계약 만료 임박은 '만료 도래' KPI가 별도 담당(중복 제거).
  const daysToEnd = c.endDate ? Math.round((new Date(c.endDate) - new Date()) / 86400000) : null;
  const hasSevereSignal = (c.signalHistory || []).some(s => s.severity === '심각');
  return { flag: hasSevereSignal || !!c.vocAttention, daysToEnd };
}
...
export function augmentRetention(data) {
  ...
  return data.map((c) => {
    const daysToEnd = c.endDate ? Math.round((new Date(c.endDate) - now) / 86400000) : null;  // endDate 없으면 null(1970 오산정 방지)
    return {
      ...c,
      openThisMonth: (c.startDate || '').slice(0, 7) === ym,
      manageNeeded: isManageNeeded(c),
      expirySoon: daysToEnd != null && daysToEnd >= 0 && daysToEnd <= 90,
